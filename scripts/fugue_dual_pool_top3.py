#!/usr/bin/env python3
"""Generate fugues from two decoding pools and keep the best merged top-K.

This script is intended for "best-of-many" workflows:
- Exploration pool (higher entropy, higher upside)
- Stability pool (lower entropy, higher floor)

It generates both pools, merges all retained candidates, ranks them, and
copies the merged top-K MIDI files into a dedicated run folder.
"""
...
def _fugue_selection_score(
    composite: float,
    cp_onset_staggering: float,
    voice_balance_ratio: float,
    onset_3or4_ratio: float,
) -> float:
    return (
        composite
        + 0.12 * cp_onset_staggering
        + 0.05 * voice_balance_ratio
        - 0.08 * onset_3or4_ratio
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dual-pool fugue generation and merged top-K ranking.")
    ...
    parser.add_argument("--key", default="D minor", help="Fugue key.")
    ...
    parser.add_argument("--texture", default="polyphonic", help="Texture conditioning token.")
    parser.add_argument("--imitation", default="high", help="Imitation conditioning token.")
    parser.add_argument("--tension", default="high", choices=["low", "moderate", "high"], help="Harmonic tension token.")
    parser.add_argument("--chromaticism", default=None, choices=["low", "moderate", "high"], help="Chromaticism token (default: form default).")
    parser.add_argument("--voices", type=int, default=4, help="Number of fugue voices.")
    parser.add_argument("--max-length", type=int, default=4096, help="Generation token budget after the prompt.")
    parser.add_argument("--candidate-batch-size", type=int, default=2, help="Sampling batch size.")
    parser.add_argument("--explore-candidates", type=int, default=120, help="Candidates in exploration pool.")
    parser.add_argument("--stable-candidates", type=int, default=80, help="Candidates in stability pool.")
    parser.add_argument("--explore-temp", type=float, default=1.00, help="Exploration pool temperature.")
    parser.add_argument("--explore-min-p", type=float, default=0.01, help="Exploration pool min-p.")
    parser.add_argument("--stable-temp", type=float, default=0.90, help="Stability pool temperature.")
    parser.add_argument("--stable-min-p", type=float, default=0.02, help="Stability pool min-p.")
    ...
    parser.add_argument("--output-root", default="output/fugue_dual_pool_runs", help="Run output root.")
    ...
    if args.explore_candidates <= 0 or args.stable_candidates <= 0:
        raise SystemExit("Both candidate counts must be > 0.")
    if args.top <= 0:
    ...
    pool_explore_dir = run_dir / "pool_explore"
    pool_stable_dir = run_dir / "pool_stable"
    merged_dir = run_dir / "merged_top"
    for p in [pool_explore_dir, pool_stable_dir, merged_dir]:
    ...
    config = {
        ...
        "stable_candidates": args.stable_candidates,
        "explore_temp": args.explore_temp,
        "explore_min_p": args.explore_min_p,
        "stable_temp": args.stable_temp,
        "stable_min_p": args.stable_min_p,
        "top": args.top,
    ...
    pools = [
        {
            ...
            "min_p": args.explore_min_p,
            "seed": args.seed_base + 1,
            "output_dir": pool_explore_dir,
        },
        {
            "name": "stable",
            "candidates": args.stable_candidates,
            "temperature": args.stable_temp,
            "min_p": args.stable_min_p,
            "seed": args.seed_base + 2,
            "output_dir": pool_stable_dir,
        },
    ]
